...
These are exact generated source mirrors of the initialized resident executable, with address/byte annotations. They are verified against the shipped binary by `tools/validate_published_signed_sources.py`; they are no longer placeholder READMEs. The canonical integrated build source remains `relocatable_source/v1_balanced/math_relocatable.asm`.
