; ============================================================================
; Firemonger C64 Math API v4 REU16M-Pareto composable + QS16 + batch-turbo profile
; ============================================================================
; Same public operand/result geometry and entry addresses as v1/v2.
; Requires a 16 MiB REU (VICE -reusize 16384) loaded with c64_math_v4_16m.reu.
; Base V4 image: first 512 KiB remains V3-compatible; V4 adds a 17-bit quarter-square
; table at REU $100000 for the new UMUL16 service.
;
...
*=$3020
math_umul16:
        ; V4 self-contained one-shot UMUL16.  V3 leaves the REC configured for
        ; fixed one-byte lookups.  Temporarily switch only LEN_LO and ACR to a
        ; four-byte sequential AUTOLOAD fetch, call the 16-bit quarter-square
        ; core, then restore V3 lookup mode before returning.
        ;
        ; Exact uniform public mean over all 2^32 pairs: 317.541031 cycles.
        lda #4
        sta REU_LEN_LO
        lda #0
        sta REU_ACR
...
*=$3a80
math_reu_qs16_begin:
        ; Mid-size V4 batch mode.  Use this when several UMUL16 calls can share
        ; the same REC configuration.  C64 destination remains MATH_REU_BYTE
        ; ($C020), LEN_HI is already zero, and the V3 lookup configuration is
        ; restored by math_reu_qs16_end.
        lda #4
        sta REU_LEN_LO
        lda #0
        sta REU_ACR
...
math_reu_qs16:
        ; --------------------------------------------------------------------
        ; V4 16x16 -> 32 unsigned multiply using a 17-bit quarter-square table
        ; in 16 MiB REU memory.
        ;
        ; Q(v)=floor(v*v/4), v=0..131070, 4 bytes/record at REU $100000.
        ; x+y and x-y always have the same parity, therefore
        ;
        ;   x*y = Q(x+y) - Q(abs(x-y))
        ;
        ; exactly, including odd operands.  Two four-byte REU FETCH/AUTOLOAD
        ; operations replace four 8x8 partial products.  Public x/y bytes are
        ; preserved.  $10-$1F are transient shared-ZP scratch.  Q(sum) is
        ; copied to $1C-$1F while Q(diff) remains at $C020-$C023.
        ;
        ; Core timing classes at this exact placement:
        ;   x1 < y1                         279 cycles
        ;   x1 > y1                         284 cycles
        ;   x1 = y1, x0 < y0                291 cycles
        ;   x1 = y1, x0 >= y0               293 cycles
        ; Exact uniform mean: 281.541031 cycles (RTS included, caller JSR out).
        ; --------------------------------------------------------------------
        clc
        lda math_x0
        adc math_y0
        ...
        ; d = abs(x-y), comparing high byte first.  Only the 1/256 high-equal
        ; slice needs the low-byte compare.
        lda math_x1
        cmp math_y1
        bcc qs16_y_gt
        bne qs16_x_ge
...
qs16_have_diff:
        ...
        ora #$10
        sta REU_BANK
        lda #REU_CMD_FETCH_AUTO
        sta REU_COMMAND

        ; Preserve Q(sum) because the second fetch reuses the same C64 buffer.
        lda MATH_REU_BYTE+0
        sta $1c
        lda MATH_REU_BYTE+1
        sta $1d
        ...
        ; REU byte address = $100000 + 4*abs(x-y).
        asl $13
        rol $14
        rol $15
        asl $13
