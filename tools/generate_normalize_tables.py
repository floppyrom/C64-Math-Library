...
L(v) = round(160 log2(v)) - 1120; a normalized major byte always has
0 <= L(major) < 160, so its high byte is implicit. Five pages map a log
...
def design_tables():
    logs = [round(160 * math.log2(max(1, v))) - 1120 for v in range(256)]
    zero = 1280
    low, high = [999.0] * 1281, [-999.0] * 1281
    cells = 0
    for major, minor, lo, hi in reduced_cells():
        ...
        cells += 1
    assert cells == 723073
    active = [i for i in range(1281) if low[i] <= high[i]]
    amin, amax = {}, {}
    for i in active:
        # Reserve rounding margin below the public 0.3621-degree / 202-LSB limits.
        amin[i], amax[i] = high[i] - .361, low[i] + .361
        ...
        amin[i] = max(amin[i], math.degrees(math.acos(min(1, ch + 199 / 32767))),
                      math.degrees(math.asin(max(0, sh - 199 / 32767))))
        amax[i] = min(amax[i], math.degrees(math.acos(max(0, cl - 199 / 32767))),
                      math.degrees(math.asin(min(1, sl + 199 / 32767))))
    ...
    data = {"scale": 160, "log_bias": 1120, "logs": logs, "indices": indices[:1280],
            "zero_index": indices[zero], "comps": comps}
    certificate = {"status": "PASS", "profiles": ["v1_balanced", "v2_pareto_fast", "v5_hybrid_lowzp"],
                   "mapping": "index=LUT[L(major)-L(minor)]; minor=0 uses zero_index; L(v)=round(160*log2(v))-1120",
...
def assembly_tables(profile, data):
    if profile in ("v3_reu_512k", "v4_reu_16m"):
        ...
        text += "; Five ratio-index pages, one log-low page, one log-high page, four component pages.\n"
        text += plane("REG_LOW", data["indices"])
        text += plane("REG_LOW+$0500", data["logs"])
