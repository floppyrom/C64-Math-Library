...
This is not two complete libraries loaded side-by-side. V5 produces **one resident library, one public I/O block and one 45-entry API**.
...
5. the complete 45-entry library still passes the common validation suite.
...
- `v5_hybrid_lowzp_reference_validation.json` / `...alternate...` — complete 45-entry validation records.
