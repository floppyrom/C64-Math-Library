...
V5 is not a second API and does not load V1 and V2 side-by-side. It is one V1-based resident image with the same 54-entry surface. The hybrid builder source-builds both donor profiles, relocates only the certified V2 kernels, and patches the existing V1 public wrapper bodies in place. No runtime profile switch or dispatcher is involved.
