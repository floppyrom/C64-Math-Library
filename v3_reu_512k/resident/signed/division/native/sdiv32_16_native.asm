; MATH_SDIV32_16 — exact native-signed executable source mirror for v3_reu_512k
; GENERATED by tools/publish_native_signed_sources.py. Do not hand-edit.
; Canonical integrated build source remains relocatable_source/v3_reu_512k/math_relocatable.asm
; This mirror contains every executable instruction reachable from this signed API after MATH_INIT.
; Immutable lookup/data tables are intentionally not duplicated here.
; Corresponding unsigned API: MATH_UDIV32_16. Executable overlap: 0 instructions.
; Public entry: $3EE0. Reachable signed instructions: 1165.
; Each instruction has an @ADDR byte annotation used by the publication validator.
!cpu 6510

; ---- executable island $2690 ----
* = $2690
L2690:
    lda $C010                          ; @2690 AD 10 C0
    sta $3E                            ; @2693 85 3E
    lda $C011                          ; @2695 AD 11 C0
    sta $3F                            ; @2698 85 3F
    lda $C012                          ; @269A AD 12 C0
    sta $40                            ; @269D 85 40
    lda $C013                          ; @269F AD 13 C0
    sta $41                            ; @26A2 85 41
    lda $C014                          ; @26A4 AD 14 C0
    sta $42                            ; @26A7 85 42
    lda $C015                          ; @26A9 AD 15 C0
    sta $43                            ; @26AC 85 43
    jsr L9200                          ; @26AE 20 00 92
    lda $4E                            ; @26B1 A5 4E
    sta $C018                          ; @26B3 8D 18 C0
    lda $4F                            ; @26B6 A5 4F
    sta $C019                          ; @26B8 8D 19 C0
    lda $48                            ; @26BB A5 48
    sta $C01A                          ; @26BD 8D 1A C0
    lda $49                            ; @26C0 A5 49
    sta $C01B                          ; @26C2 8D 1B C0
    lda $4A                            ; @26C5 A5 4A
    sta $C01C                          ; @26C7 8D 1C C0
    lda $4B                            ; @26CA A5 4B
    sta $C01D                          ; @26CC 8D 1D C0
    rts                                ; @26CF 60
; ---- executable island $3EE0 ----
* = $3EE0
L3EE0:
    jmp L2690                          ; @3EE0 4C 90 26
; ---- executable island $890C ----
* = $890C
L890C:
    !byte $A7, $45    ; LAX zp         ; @890C A7 45
    cmp $47                            ; @890E C5 47
    bcs L8939                          ; @8910 B0 27
L8912:
    lda #$00                           ; @8912 A9 00
    sta $48                            ; @8914 85 48
    sta $49                            ; @8916 85 49
    lda $44                            ; @8918 A5 44
    sta $4A                            ; @891A 85 4A
    stx $4B                            ; @891C 86 4B
    rts                                ; @891E 60
L891F:
    lda $44                            ; @891F A5 44
    sbc $46                            ; @8921 E5 46
    bcc L8912                          ; @8923 90 ED
    cpx #$01                           ; @8925 E0 01
    bcc L8936                          ; @8927 90 0D
    sta $4A                            ; @8929 85 4A
    !byte $0B, $00    ; ANC #imm       ; @892B 0B 00
    sta $4B                            ; @892D 85 4B
    sta $49                            ; @892F 85 49
    lda #$01                           ; @8931 A9 01
    sta $48                            ; @8933 85 48
    rts                                ; @8935 60
L8936:
    jmp L8D0E                          ; @8936 4C 0E 8D
L8939:
    beq L891F                          ; @8939 F0 E4
    lda $44                            ; @893B A5 44
    sbc $46                            ; @893D E5 46
    sta $4A                            ; @893F 85 4A
    txa                                ; @8941 8A
    sbc $47                            ; @8942 E5 47
    cmp $47                            ; @8944 C5 47
    bcs L89BB                          ; @8946 B0 73
    sta $4B                            ; @8948 85 4B
    lda #$00                           ; @894A A9 00
    sta $49                            ; @894C 85 49
    lda #$01                           ; @894E A9 01
    sta $48                            ; @8950 85 48
    rts                                ; @8952 60
L8953:
    cmp $8D4D,x                        ; @8953 DD 4D 8D
    bcc L895B                          ; @8956 90 03
    jmp L8BFA                          ; @8958 4C FA 8B
L895B:
    sec                                ; @895B 38
    bcs L89C3                          ; @895C B0 65
L895E:
    lda $4A                            ; @895E A5 4A
    sbc $46                            ; @8960 E5 46
    bcs L896C                          ; @8962 B0 08
    txa                                ; @8964 8A
L8965:
    sta $4B                            ; @8965 85 4B
    lda #$02                           ; @8967 A9 02
    sta $48                            ; @8969 85 48
    rts                                ; @896B 60
L896C:
    sta $4A                            ; @896C 85 4A
    !byte $0B, $00    ; ANC #imm       ; @896E 0B 00
    bcc L8979                          ; @8970 90 07
L8972:
    lda $4A                            ; @8972 A5 4A
    sbc $46                            ; @8974 E5 46
    bcs L8980                          ; @8976 B0 08
    txa                                ; @8978 8A
L8979:
    sta $4B                            ; @8979 85 4B
    lda #$03                           ; @897B A9 03
    sta $48                            ; @897D 85 48
    rts                                ; @897F 60
L8980:
    sta $4A                            ; @8980 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8982 0B 00
    bcc L898D                          ; @8984 90 07
L8986:
    lda $4A                            ; @8986 A5 4A
    sbc $46                            ; @8988 E5 46
    bcs L8994                          ; @898A B0 08
    txa                                ; @898C 8A
L898D:
    sta $4B                            ; @898D 85 4B
    lda #$04                           ; @898F A9 04
    sta $48                            ; @8991 85 48
    rts                                ; @8993 60
L8994:
    sta $4A                            ; @8994 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8996 0B 00
    jmp L8A7B                          ; @8998 4C 7B 8A
L899B:
    tax                                ; @899B AA
    lda $4A                            ; @899C A5 4A
    sbc $46                            ; @899E E5 46
    bcs L89AD                          ; @89A0 B0 0B
    stx $4B                            ; @89A2 86 4B
    lda #$00                           ; @89A4 A9 00
    sta $49                            ; @89A6 85 49
    lda #$01                           ; @89A8 A9 01
    sta $48                            ; @89AA 85 48
    rts                                ; @89AC 60
L89AD:
    dex                                ; @89AD CA
    bpl L89B3                          ; @89AE 10 03
    jmp L8D0E                          ; @89B0 4C 0E 8D
L89B3:
    sta $4A                            ; @89B3 85 4A
    !byte $0B, $00    ; ANC #imm       ; @89B5 0B 00
    sta $49                            ; @89B7 85 49
    bcc L8965                          ; @89B9 90 AA
L89BB:
    beq L899B                          ; @89BB F0 DE
    ldx $47                            ; @89BD A6 47
    cpx #$0F                           ; @89BF E0 0F
    bcc L8953                          ; @89C1 90 90
L89C3:
    ldy #$00                           ; @89C3 A0 00
    sty $49                            ; @89C5 84 49
    tay                                ; @89C7 A8
    lda $4A                            ; @89C8 A5 4A
    sbc $46                            ; @89CA E5 46
    sta $4A                            ; @89CC 85 4A
    tya                                ; @89CE 98
    sbc $47                            ; @89CF E5 47
    cmp $47                            ; @89D1 C5 47
    bcc L8965                          ; @89D3 90 90
    beq L895E                          ; @89D5 F0 87
    tay                                ; @89D7 A8
    lda $4A                            ; @89D8 A5 4A
    sbc $46                            ; @89DA E5 46
    sta $4A                            ; @89DC 85 4A
    tya                                ; @89DE 98
    sbc $47                            ; @89DF E5 47
    cmp $47                            ; @89E1 C5 47
    bcc L8979                          ; @89E3 90 94
    beq L8972                          ; @89E5 F0 8B
    tay                                ; @89E7 A8
    lda $4A                            ; @89E8 A5 4A
    sbc $46                            ; @89EA E5 46
    sta $4A                            ; @89EC 85 4A
    tya                                ; @89EE 98
    sbc $47                            ; @89EF E5 47
    cmp $47                            ; @89F1 C5 47
    bcc L898D                          ; @89F3 90 98
    beq L8986                          ; @89F5 F0 8F
    tay                                ; @89F7 A8
    lda $4A                            ; @89F8 A5 4A
    sbc $46                            ; @89FA E5 46
    sta $4A                            ; @89FC 85 4A
    tya                                ; @89FE 98
    sbc $47                            ; @89FF E5 47
    cmp $47                            ; @8A01 C5 47
    bcc L8A7B                          ; @8A03 90 76
    beq L8A74                          ; @8A05 F0 6D
    tay                                ; @8A07 A8
    lda $4A                            ; @8A08 A5 4A
    sbc $46                            ; @8A0A E5 46
    sta $4A                            ; @8A0C 85 4A
    tya                                ; @8A0E 98
    sbc $47                            ; @8A0F E5 47
    cmp $47                            ; @8A11 C5 47
    bcc L8A89                          ; @8A13 90 74
    beq L8A82                          ; @8A15 F0 6B
    tay                                ; @8A17 A8
    lda $4A                            ; @8A18 A5 4A
    sbc $46                            ; @8A1A E5 46
    sta $4A                            ; @8A1C 85 4A
    tya                                ; @8A1E 98
    sbc $47                            ; @8A1F E5 47
    cmp $47                            ; @8A21 C5 47
    bcc L8A97                          ; @8A23 90 72
    beq L8A90                          ; @8A25 F0 69
    tay                                ; @8A27 A8
    lda $4A                            ; @8A28 A5 4A
    sbc $46                            ; @8A2A E5 46
    sta $4A                            ; @8A2C 85 4A
    tya                                ; @8A2E 98
    sbc $47                            ; @8A2F E5 47
    cmp $47                            ; @8A31 C5 47
    bcc L8AA5                          ; @8A33 90 70
    beq L8A9E                          ; @8A35 F0 67
    tay                                ; @8A37 A8
    lda $4A                            ; @8A38 A5 4A
    sbc $46                            ; @8A3A E5 46
    sta $4A                            ; @8A3C 85 4A
    tya                                ; @8A3E 98
    sbc $47                            ; @8A3F E5 47
    cmp $47                            ; @8A41 C5 47
    bcc L8AB3                          ; @8A43 90 6E
    beq L8AAC                          ; @8A45 F0 65
    tay                                ; @8A47 A8
    lda $4A                            ; @8A48 A5 4A
    sbc $46                            ; @8A4A E5 46
    sta $4A                            ; @8A4C 85 4A
    tya                                ; @8A4E 98
    sbc $47                            ; @8A4F E5 47
    cmp $47                            ; @8A51 C5 47
    bcc L8AC1                          ; @8A53 90 6C
    beq L8ABA                          ; @8A55 F0 63
    tay                                ; @8A57 A8
    lda $4A                            ; @8A58 A5 4A
    sbc $46                            ; @8A5A E5 46
    sta $4A                            ; @8A5C 85 4A
    tya                                ; @8A5E 98
    sbc $47                            ; @8A5F E5 47
    cmp $47                            ; @8A61 C5 47
    bcc L8ACF                          ; @8A63 90 6A
    beq L8AC8                          ; @8A65 F0 61
    tay                                ; @8A67 A8
    lda $4A                            ; @8A68 A5 4A
    sbc $46                            ; @8A6A E5 46
    sta $4A                            ; @8A6C 85 4A
    tya                                ; @8A6E 98
    sbc $47                            ; @8A6F E5 47
    jmp L8B00                          ; @8A71 4C 00 8B
L8A74:
    lda $4A                            ; @8A74 A5 4A
    sbc $46                            ; @8A76 E5 46
    bcs L8AD6                          ; @8A78 B0 5C
    txa                                ; @8A7A 8A
L8A7B:
    sta $4B                            ; @8A7B 85 4B
    lda #$05                           ; @8A7D A9 05
    sta $48                            ; @8A7F 85 48
    rts                                ; @8A81 60
L8A82:
    lda $4A                            ; @8A82 A5 4A
    sbc $46                            ; @8A84 E5 46
    bcs L8ADC                          ; @8A86 B0 54
    txa                                ; @8A88 8A
L8A89:
    sta $4B                            ; @8A89 85 4B
    lda #$06                           ; @8A8B A9 06
    sta $48                            ; @8A8D 85 48
    rts                                ; @8A8F 60
L8A90:
    lda $4A                            ; @8A90 A5 4A
    sbc $46                            ; @8A92 E5 46
    bcs L8AE2                          ; @8A94 B0 4C
    txa                                ; @8A96 8A
L8A97:
    sta $4B                            ; @8A97 85 4B
    lda #$07                           ; @8A99 A9 07
    sta $48                            ; @8A9B 85 48
    rts                                ; @8A9D 60
L8A9E:
    lda $4A                            ; @8A9E A5 4A
    sbc $46                            ; @8AA0 E5 46
    bcs L8AE8                          ; @8AA2 B0 44
    txa                                ; @8AA4 8A
L8AA5:
    sta $4B                            ; @8AA5 85 4B
    lda #$08                           ; @8AA7 A9 08
    sta $48                            ; @8AA9 85 48
    rts                                ; @8AAB 60
L8AAC:
    lda $4A                            ; @8AAC A5 4A
    sbc $46                            ; @8AAE E5 46
    bcs L8AEE                          ; @8AB0 B0 3C
    txa                                ; @8AB2 8A
L8AB3:
    sta $4B                            ; @8AB3 85 4B
    lda #$09                           ; @8AB5 A9 09
    sta $48                            ; @8AB7 85 48
    rts                                ; @8AB9 60
L8ABA:
    lda $4A                            ; @8ABA A5 4A
    sbc $46                            ; @8ABC E5 46
    bcs L8AF4                          ; @8ABE B0 34
    txa                                ; @8AC0 8A
L8AC1:
    sta $4B                            ; @8AC1 85 4B
    lda #$0A                           ; @8AC3 A9 0A
    sta $48                            ; @8AC5 85 48
    rts                                ; @8AC7 60
L8AC8:
    lda $4A                            ; @8AC8 A5 4A
    sbc $46                            ; @8ACA E5 46
    bcs L8AFA                          ; @8ACC B0 2C
    txa                                ; @8ACE 8A
L8ACF:
    sta $4B                            ; @8ACF 85 4B
    lda #$0B                           ; @8AD1 A9 0B
    sta $48                            ; @8AD3 85 48
    rts                                ; @8AD5 60
L8AD6:
    sta $4A                            ; @8AD6 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8AD8 0B 00
    bcc L8A89                          ; @8ADA 90 AD
L8ADC:
    sta $4A                            ; @8ADC 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8ADE 0B 00
    bcc L8A97                          ; @8AE0 90 B5
L8AE2:
    sta $4A                            ; @8AE2 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8AE4 0B 00
    bcc L8AA5                          ; @8AE6 90 BD
L8AE8:
    sta $4A                            ; @8AE8 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8AEA 0B 00
    bcc L8AB3                          ; @8AEC 90 C5
L8AEE:
    sta $4A                            ; @8AEE 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8AF0 0B 00
    bcc L8AC1                          ; @8AF2 90 CD
L8AF4:
    sta $4A                            ; @8AF4 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8AF6 0B 00
    bcc L8ACF                          ; @8AF8 90 D5
L8AFA:
    sta $4A                            ; @8AFA 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8AFC 0B 00
    bcc L8B57                          ; @8AFE 90 57
L8B00:
    cmp $47                            ; @8B00 C5 47
    bcc L8B57                          ; @8B02 90 53
    beq L8B50                          ; @8B04 F0 4A
    tay                                ; @8B06 A8
    lda $4A                            ; @8B07 A5 4A
    sbc $46                            ; @8B09 E5 46
    sta $4A                            ; @8B0B 85 4A
    tya                                ; @8B0D 98
    sbc $47                            ; @8B0E E5 47
    cmp $47                            ; @8B10 C5 47
    bcc L8B6B                          ; @8B12 90 57
    beq L8B64                          ; @8B14 F0 4E
    tay                                ; @8B16 A8
    lda $4A                            ; @8B17 A5 4A
    sbc $46                            ; @8B19 E5 46
    sta $4A                            ; @8B1B 85 4A
    tya                                ; @8B1D 98
    sbc $47                            ; @8B1E E5 47
    cmp $47                            ; @8B20 C5 47
    bcc L8B7F                          ; @8B22 90 5B
    beq L8B78                          ; @8B24 F0 52
    tay                                ; @8B26 A8
    lda $4A                            ; @8B27 A5 4A
    sbc $46                            ; @8B29 E5 46
    sta $4A                            ; @8B2B 85 4A
    tya                                ; @8B2D 98
    sbc $47                            ; @8B2E E5 47
    cmp $47                            ; @8B30 C5 47
    bcc L8B93                          ; @8B32 90 5F
    beq L8B8C                          ; @8B34 F0 56
    tay                                ; @8B36 A8
    lda $4A                            ; @8B37 A5 4A
    sbc $46                            ; @8B39 E5 46
    sta $4A                            ; @8B3B 85 4A
    tya                                ; @8B3D 98
    sbc $47                            ; @8B3E E5 47
    cmp $47                            ; @8B40 C5 47
    bcc L8BA7                          ; @8B42 90 63
    beq L8BA0                          ; @8B44 F0 5A
    tay                                ; @8B46 A8
    lda $4A                            ; @8B47 A5 4A
    sbc $46                            ; @8B49 E5 46
    sta $4A                            ; @8B4B 85 4A
    tya                                ; @8B4D 98
    sbc $47                            ; @8B4E E5 47
L8B50:
    lda $4A                            ; @8B50 A5 4A
    sbc $46                            ; @8B52 E5 46
    bcs L8B5E                          ; @8B54 B0 08
    txa                                ; @8B56 8A
L8B57:
    sta $4B                            ; @8B57 85 4B
    lda #$0C                           ; @8B59 A9 0C
    sta $48                            ; @8B5B 85 48
    rts                                ; @8B5D 60
L8B5E:
    sta $4A                            ; @8B5E 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8B60 0B 00
    bcc L8B6B                          ; @8B62 90 07
L8B64:
    lda $4A                            ; @8B64 A5 4A
    sbc $46                            ; @8B66 E5 46
    bcs L8B72                          ; @8B68 B0 08
    txa                                ; @8B6A 8A
L8B6B:
    sta $4B                            ; @8B6B 85 4B
    lda #$0D                           ; @8B6D A9 0D
    sta $48                            ; @8B6F 85 48
    rts                                ; @8B71 60
L8B72:
    sta $4A                            ; @8B72 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8B74 0B 00
    bcc L8B7F                          ; @8B76 90 07
L8B78:
    lda $4A                            ; @8B78 A5 4A
    sbc $46                            ; @8B7A E5 46
    bcs L8B86                          ; @8B7C B0 08
    txa                                ; @8B7E 8A
L8B7F:
    sta $4B                            ; @8B7F 85 4B
    lda #$0E                           ; @8B81 A9 0E
    sta $48                            ; @8B83 85 48
    rts                                ; @8B85 60
L8B86:
    sta $4A                            ; @8B86 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8B88 0B 00
    bcc L8B93                          ; @8B8A 90 07
L8B8C:
    lda $4A                            ; @8B8C A5 4A
    sbc $46                            ; @8B8E E5 46
    bcs L8B9A                          ; @8B90 B0 08
    txa                                ; @8B92 8A
L8B93:
    sta $4B                            ; @8B93 85 4B
    lda #$0F                           ; @8B95 A9 0F
    sta $48                            ; @8B97 85 48
    rts                                ; @8B99 60
L8B9A:
    sta $4A                            ; @8B9A 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8B9C 0B 00
    bcc L8BA7                          ; @8B9E 90 07
L8BA0:
    lda $4A                            ; @8BA0 A5 4A
    sbc $46                            ; @8BA2 E5 46
    bcs L8BAE                          ; @8BA4 B0 08
    txa                                ; @8BA6 8A
L8BA7:
    sta $4B                            ; @8BA7 85 4B
    lda #$10                           ; @8BA9 A9 10
    sta $48                            ; @8BAB 85 48
    rts                                ; @8BAD 60
L8BAE:
    sta $4A                            ; @8BAE 85 4A
    !byte $0B, $00    ; ANC #imm       ; @8BB0 0B 00
    sta $4B                            ; @8BB2 85 4B
    lda #$11                           ; @8BB4 A9 11
    sta $48                            ; @8BB6 85 48
    rts                                ; @8BB8 60
L8BB9:
    jmp L8D12                          ; @8BB9 4C 12 8D
L8BBC:
    jmp L8D0E                          ; @8BBC 4C 0E 8D
L8BBF:
    cpx #$01                           ; @8BBF E0 01
    bcc L8BBC                          ; @8BC1 90 F9
    beq L8BB9                          ; @8BC3 F0 F4
    lda $44                            ; @8BC5 A5 44
    asl a                              ; @8BC7 0A
    sta $48                            ; @8BC8 85 48
    lda $45                            ; @8BCA A5 45
    rol a                              ; @8BCC 2A
    sta $4A                            ; @8BCD 85 4A
    lda #$00                           ; @8BCF A9 00
    sta $49                            ; @8BD1 85 49
    rol a                              ; @8BD3 2A
    rol $48                            ; @8BD4 26 48
    rol $4A                            ; @8BD6 26 4A
    rol a                              ; @8BD8 2A
    cmp $47                            ; @8BD9 C5 47
    bcc L8C16                          ; @8BDB 90 39
    beq L8BEB                          ; @8BDD F0 0C
    tay                                ; @8BDF A8
    lda $4A                            ; @8BE0 A5 4A
    sbc $46                            ; @8BE2 E5 46
    sta $4A                            ; @8BE4 85 4A
    tya                                ; @8BE6 98
    sbc $47                            ; @8BE7 E5 47
    bcs L8C16                          ; @8BE9 B0 2B
L8BEB:
    lda $4A                            ; @8BEB A5 4A
    sbc $46                            ; @8BED E5 46
    bcc L8BF7                          ; @8BEF 90 06
    sta $4A                            ; @8BF1 85 4A
    lda #$00                           ; @8BF3 A9 00
    bcs L8C16                          ; @8BF5 B0 1F
L8BF7:
    txa                                ; @8BF7 8A
    bcc L8C16                          ; @8BF8 90 1C
L8BFA:
    cpx #$04                           ; @8BFA E0 04
    bcc L8BBF                          ; @8BFC 90 C1
    cpx #$08                           ; @8BFE E0 08
    bcs L8C4B                          ; @8C00 B0 49
    lda $44                            ; @8C02 A5 44
    asl a                              ; @8C04 0A
    sta $48                            ; @8C05 85 48
    lda $45                            ; @8C07 A5 45
    rol a                              ; @8C09 2A
    sta $4A                            ; @8C0A 85 4A
    lda #$00                           ; @8C0C A9 00
    sta $49                            ; @8C0E 85 49
    rol a                              ; @8C10 2A
    asl $48                            ; @8C11 06 48
    rol $4A                            ; @8C13 26 4A
    rol a                              ; @8C15 2A
L8C16:
    rol $48                            ; @8C16 26 48
    rol $4A                            ; @8C18 26 4A
    rol a                              ; @8C1A 2A
    cmp $47                            ; @8C1B C5 47
    bcc L8C64                          ; @8C1D 90 45
    beq L8C2D                          ; @8C1F F0 0C
    tay                                ; @8C21 A8
    lda $4A                            ; @8C22 A5 4A
    sbc $46                            ; @8C24 E5 46
    sta $4A                            ; @8C26 85 4A
    tya                                ; @8C28 98
    sbc $47                            ; @8C29 E5 47
    bcs L8C64                          ; @8C2B B0 37
L8C2D:
    lda $4A                            ; @8C2D A5 4A
    sbc $46                            ; @8C2F E5 46
    bcc L8C39                          ; @8C31 90 06
    sta $4A                            ; @8C33 85 4A
    lda #$00                           ; @8C35 A9 00
    bcs L8C64                          ; @8C37 B0 2B
L8C39:
    txa                                ; @8C39 8A
    bcc L8C64                          ; @8C3A 90 28
L8C3C:
    lda $4A                            ; @8C3C A5 4A
    sbc $46                            ; @8C3E E5 46
    bcc L8C48                          ; @8C40 90 06
    sta $4A                            ; @8C42 85 4A
    lda #$00                           ; @8C44 A9 00
    bcs L8C79                          ; @8C46 B0 31
L8C48:
    txa                                ; @8C48 8A
    bcc L8C79                          ; @8C49 90 2E
L8C4B:
    lda $44                            ; @8C4B A5 44
    asl a                              ; @8C4D 0A
    sta $48                            ; @8C4E 85 48
    lda $45                            ; @8C50 A5 45
    rol a                              ; @8C52 2A
    sta $4A                            ; @8C53 85 4A
    lda #$00                           ; @8C55 A9 00
    sta $49                            ; @8C57 85 49
    rol a                              ; @8C59 2A
    asl $48                            ; @8C5A 06 48
    rol $4A                            ; @8C5C 26 4A
    rol a                              ; @8C5E 2A
    asl $48                            ; @8C5F 06 48
    rol $4A                            ; @8C61 26 4A
    rol a                              ; @8C63 2A
L8C64:
    rol $48                            ; @8C64 26 48
    rol $4A                            ; @8C66 26 4A
    rol a                              ; @8C68 2A
    cmp $47                            ; @8C69 C5 47
    bcc L8C79                          ; @8C6B 90 0C
    beq L8C3C                          ; @8C6D F0 CD
    tay                                ; @8C6F A8
    lda $4A                            ; @8C70 A5 4A
    sbc $46                            ; @8C72 E5 46
    sta $4A                            ; @8C74 85 4A
    tya                                ; @8C76 98
    sbc $47                            ; @8C77 E5 47
L8C79:
    rol $48                            ; @8C79 26 48
    rol $4A                            ; @8C7B 26 4A
    rol a                              ; @8C7D 2A
    cmp $47                            ; @8C7E C5 47
    bcc L8C8E                          ; @8C80 90 0C
    beq L8CD2                          ; @8C82 F0 4E
    tay                                ; @8C84 A8
    lda $4A                            ; @8C85 A5 4A
    sbc $46                            ; @8C87 E5 46
    sta $4A                            ; @8C89 85 4A
    tya                                ; @8C8B 98
    sbc $47                            ; @8C8C E5 47
L8C8E:
    rol $48                            ; @8C8E 26 48
    rol $4A                            ; @8C90 26 4A
    rol a                              ; @8C92 2A
    cmp $47                            ; @8C93 C5 47
    bcc L8CA3                          ; @8C95 90 0C
    beq L8CE1                          ; @8C97 F0 48
    tay                                ; @8C99 A8
    lda $4A                            ; @8C9A A5 4A
    sbc $46                            ; @8C9C E5 46
    sta $4A                            ; @8C9E 85 4A
    tya                                ; @8CA0 98
    sbc $47                            ; @8CA1 E5 47
L8CA3:
    rol $48                            ; @8CA3 26 48
    rol $4A                            ; @8CA5 26 4A
    rol a                              ; @8CA7 2A
    cmp $47                            ; @8CA8 C5 47
    bcc L8CB8                          ; @8CAA 90 0C
    beq L8CF0                          ; @8CAC F0 42
    tay                                ; @8CAE A8
    lda $4A                            ; @8CAF A5 4A
    sbc $46                            ; @8CB1 E5 46
    sta $4A                            ; @8CB3 85 4A
    tya                                ; @8CB5 98
    sbc $47                            ; @8CB6 E5 47
L8CB8:
    rol $48                            ; @8CB8 26 48
    rol $4A                            ; @8CBA 26 4A
    rol a                              ; @8CBC 2A
    cmp $47                            ; @8CBD C5 47
    bcc L8CCD                          ; @8CBF 90 0C
    beq L8CFF                          ; @8CC1 F0 3C
    tay                                ; @8CC3 A8
    lda $4A                            ; @8CC4 A5 4A
    sbc $46                            ; @8CC6 E5 46
    sta $4A                            ; @8CC8 85 4A
    tya                                ; @8CCA 98
    sbc $47                            ; @8CCB E5 47
L8CCD:
    rol $48                            ; @8CCD 26 48
    sta $4B                            ; @8CCF 85 4B
    rts                                ; @8CD1 60
L8CD2:
    lda $4A                            ; @8CD2 A5 4A
    sbc $46                            ; @8CD4 E5 46
    bcc L8CDE                          ; @8CD6 90 06
    sta $4A                            ; @8CD8 85 4A
    lda #$00                           ; @8CDA A9 00
    bcs L8C8E                          ; @8CDC B0 B0
L8CDE:
    txa                                ; @8CDE 8A
    bcc L8C8E                          ; @8CDF 90 AD
L8CE1:
    lda $4A                            ; @8CE1 A5 4A
    sbc $46                            ; @8CE3 E5 46
    bcc L8CED                          ; @8CE5 90 06
    sta $4A                            ; @8CE7 85 4A
    lda #$00                           ; @8CE9 A9 00
    bcs L8CA3                          ; @8CEB B0 B6
L8CED:
    txa                                ; @8CED 8A
    bcc L8CA3                          ; @8CEE 90 B3
L8CF0:
    lda $4A                            ; @8CF0 A5 4A
    sbc $46                            ; @8CF2 E5 46
    bcc L8CFC                          ; @8CF4 90 06
    sta $4A                            ; @8CF6 85 4A
    lda #$00                           ; @8CF8 A9 00
    bcs L8CB8                          ; @8CFA B0 BC
L8CFC:
    txa                                ; @8CFC 8A
    bcc L8CB8                          ; @8CFD 90 B9
L8CFF:
    lda $4A                            ; @8CFF A5 4A
    sbc $46                            ; @8D01 E5 46
    bcc L8D0B                          ; @8D03 90 06
    sta $4A                            ; @8D05 85 4A
    lda #$00                           ; @8D07 A9 00
    bcs L8CCD                          ; @8D09 B0 C2
L8D0B:
    txa                                ; @8D0B 8A
    bcc L8CCD                          ; @8D0C 90 BF
L8D0E:
    lda $46                            ; @8D0E A5 46
    beq L8D43                          ; @8D10 F0 31
L8D12:
    lda $44                            ; @8D12 A5 44
    sta $48                            ; @8D14 85 48
    lda $45                            ; @8D16 A5 45
    sta $49                            ; @8D18 85 49
    lda #$00                           ; @8D1A A9 00
    lsr a                              ; @8D1C 4A
    sta $4A                            ; @8D1D 85 4A
    sta $4B                            ; @8D1F 85 4B
    ldy #$10                           ; @8D21 A0 10
L8D23:
    rol $48                            ; @8D23 26 48
    rol $49                            ; @8D25 26 49
    rol $4A                            ; @8D27 26 4A
    rol $4B                            ; @8D29 26 4B
    sec                                ; @8D2B 38
    lda $4A                            ; @8D2C A5 4A
    sbc $46                            ; @8D2E E5 46
    tax                                ; @8D30 AA
    lda $4B                            ; @8D31 A5 4B
    sbc $47                            ; @8D33 E5 47
    bcc L8D3B                          ; @8D35 90 04
    sta $4B                            ; @8D37 85 4B
    stx $4A                            ; @8D39 86 4A
L8D3B:
    dey                                ; @8D3B 88
    bne L8D23                          ; @8D3C D0 E5
    rol $48                            ; @8D3E 26 48
    rol $49                            ; @8D40 26 49
    rts                                ; @8D42 60
L8D43:
    sta $48                            ; @8D43 85 48
    sta $49                            ; @8D45 85 49
    sta $4A                            ; @8D47 85 4A
    sta $4B                            ; @8D49 85 4B
    sec                                ; @8D4B 38
    rts                                ; @8D4C 60
; ---- executable island $8E00 ----
* = $8E00
L8E00:
    jsr L890C                          ; @8E00 20 0C 89
    bcc L8E08                          ; @8E03 90 03
    jmp L90DA                          ; @8E05 4C DA 90
L8E08:
    lda $4C                            ; @8E08 A5 4C
    sta $4E                            ; @8E0A 85 4E
    lda $4D                            ; @8E0C A5 4D
    sta $4F                            ; @8E0E 85 4F
    lda $4B                            ; @8E10 A5 4B
    rol $4E                            ; @8E12 26 4E
    rol $4F                            ; @8E14 26 4F
    rol $4A                            ; @8E16 26 4A
    rol a                              ; @8E18 2A
    bcs L8E33                          ; @8E19 B0 18
    cmp $47                            ; @8E1B C5 47
    bcc L8E3E                          ; @8E1D 90 1F
    bne L8E27                          ; @8E1F D0 06
    ldx $4A                            ; @8E21 A6 4A
    cpx $46                            ; @8E23 E4 46
    bcc L8E3E                          ; @8E25 90 17
L8E27:
    tax                                ; @8E27 AA
    lda $4A                            ; @8E28 A5 4A
    sbc $46                            ; @8E2A E5 46
    sta $4A                            ; @8E2C 85 4A
    txa                                ; @8E2E 8A
    sbc $47                            ; @8E2F E5 47
    bcs L8E3E                          ; @8E31 B0 0B
L8E33:
    tax                                ; @8E33 AA
    lda $4A                            ; @8E34 A5 4A
    sbc $46                            ; @8E36 E5 46
    sta $4A                            ; @8E38 85 4A
    txa                                ; @8E3A 8A
    sbc $47                            ; @8E3B E5 47
    sec                                ; @8E3D 38
L8E3E:
    rol $4E                            ; @8E3E 26 4E
    rol $4F                            ; @8E40 26 4F
    rol $4A                            ; @8E42 26 4A
    rol a                              ; @8E44 2A
    bcs L8E5F                          ; @8E45 B0 18
    cmp $47                            ; @8E47 C5 47
    bcc L8E6A                          ; @8E49 90 1F
    bne L8E53                          ; @8E4B D0 06
    ldx $4A                            ; @8E4D A6 4A
    cpx $46                            ; @8E4F E4 46
    bcc L8E6A                          ; @8E51 90 17
L8E53:
    tax                                ; @8E53 AA
    lda $4A                            ; @8E54 A5 4A
    sbc $46                            ; @8E56 E5 46
    sta $4A                            ; @8E58 85 4A
    txa                                ; @8E5A 8A
    sbc $47                            ; @8E5B E5 47
    bcs L8E6A                          ; @8E5D B0 0B
L8E5F:
    tax                                ; @8E5F AA
    lda $4A                            ; @8E60 A5 4A
    sbc $46                            ; @8E62 E5 46
    sta $4A                            ; @8E64 85 4A
    txa                                ; @8E66 8A
    sbc $47                            ; @8E67 E5 47
    sec                                ; @8E69 38
L8E6A:
    rol $4E                            ; @8E6A 26 4E
    rol $4F                            ; @8E6C 26 4F
    rol $4A                            ; @8E6E 26 4A
    rol a                              ; @8E70 2A
    bcs L8E8B                          ; @8E71 B0 18
    cmp $47                            ; @8E73 C5 47
    bcc L8E96                          ; @8E75 90 1F
    bne L8E7F                          ; @8E77 D0 06
    ldx $4A                            ; @8E79 A6 4A
    cpx $46                            ; @8E7B E4 46
    bcc L8E96                          ; @8E7D 90 17
L8E7F:
    tax                                ; @8E7F AA
    lda $4A                            ; @8E80 A5 4A
    sbc $46                            ; @8E82 E5 46
    sta $4A                            ; @8E84 85 4A
    txa                                ; @8E86 8A
    sbc $47                            ; @8E87 E5 47
    bcs L8E96                          ; @8E89 B0 0B
L8E8B:
    tax                                ; @8E8B AA
    lda $4A                            ; @8E8C A5 4A
    sbc $46                            ; @8E8E E5 46
    sta $4A                            ; @8E90 85 4A
    txa                                ; @8E92 8A
    sbc $47                            ; @8E93 E5 47
    sec                                ; @8E95 38
L8E96:
    rol $4E                            ; @8E96 26 4E
    rol $4F                            ; @8E98 26 4F
    rol $4A                            ; @8E9A 26 4A
    rol a                              ; @8E9C 2A
    bcs L8EB7                          ; @8E9D B0 18
    cmp $47                            ; @8E9F C5 47
    bcc L8EC2                          ; @8EA1 90 1F
    bne L8EAB                          ; @8EA3 D0 06
    ldx $4A                            ; @8EA5 A6 4A
    cpx $46                            ; @8EA7 E4 46
    bcc L8EC2                          ; @8EA9 90 17
L8EAB:
    tax                                ; @8EAB AA
    lda $4A                            ; @8EAC A5 4A
    sbc $46                            ; @8EAE E5 46
    sta $4A                            ; @8EB0 85 4A
    txa                                ; @8EB2 8A
    sbc $47                            ; @8EB3 E5 47
    bcs L8EC2                          ; @8EB5 B0 0B
L8EB7:
    tax                                ; @8EB7 AA
    lda $4A                            ; @8EB8 A5 4A
    sbc $46                            ; @8EBA E5 46
    sta $4A                            ; @8EBC 85 4A
    txa                                ; @8EBE 8A
    sbc $47                            ; @8EBF E5 47
    sec                                ; @8EC1 38
L8EC2:
    rol $4E                            ; @8EC2 26 4E
    rol $4F                            ; @8EC4 26 4F
    rol $4A                            ; @8EC6 26 4A
    rol a                              ; @8EC8 2A
    bcs L8EE3                          ; @8EC9 B0 18
    cmp $47                            ; @8ECB C5 47
    bcc L8EEE                          ; @8ECD 90 1F
    bne L8ED7                          ; @8ECF D0 06
    ldx $4A                            ; @8ED1 A6 4A
    cpx $46                            ; @8ED3 E4 46
    bcc L8EEE                          ; @8ED5 90 17
L8ED7:
    tax                                ; @8ED7 AA
    lda $4A                            ; @8ED8 A5 4A
    sbc $46                            ; @8EDA E5 46
    sta $4A                            ; @8EDC 85 4A
    txa                                ; @8EDE 8A
    sbc $47                            ; @8EDF E5 47
    bcs L8EEE                          ; @8EE1 B0 0B
L8EE3:
    tax                                ; @8EE3 AA
    lda $4A                            ; @8EE4 A5 4A
    sbc $46                            ; @8EE6 E5 46
    sta $4A                            ; @8EE8 85 4A
    txa                                ; @8EEA 8A
    sbc $47                            ; @8EEB E5 47
    sec                                ; @8EED 38
L8EEE:
    rol $4E                            ; @8EEE 26 4E
    rol $4F                            ; @8EF0 26 4F
    rol $4A                            ; @8EF2 26 4A
    rol a                              ; @8EF4 2A
    bcs L8F0F                          ; @8EF5 B0 18
    cmp $47                            ; @8EF7 C5 47
    bcc L8F1A                          ; @8EF9 90 1F
    bne L8F03                          ; @8EFB D0 06
    ldx $4A                            ; @8EFD A6 4A
    cpx $46                            ; @8EFF E4 46
    bcc L8F1A                          ; @8F01 90 17
L8F03:
    tax                                ; @8F03 AA
    lda $4A                            ; @8F04 A5 4A
    sbc $46                            ; @8F06 E5 46
    sta $4A                            ; @8F08 85 4A
    txa                                ; @8F0A 8A
    sbc $47                            ; @8F0B E5 47
    bcs L8F1A                          ; @8F0D B0 0B
L8F0F:
    tax                                ; @8F0F AA
    lda $4A                            ; @8F10 A5 4A
    sbc $46                            ; @8F12 E5 46
    sta $4A                            ; @8F14 85 4A
    txa                                ; @8F16 8A
    sbc $47                            ; @8F17 E5 47
    sec                                ; @8F19 38
L8F1A:
    rol $4E                            ; @8F1A 26 4E
    rol $4F                            ; @8F1C 26 4F
    rol $4A                            ; @8F1E 26 4A
    rol a                              ; @8F20 2A
    bcs L8F3B                          ; @8F21 B0 18
    cmp $47                            ; @8F23 C5 47
    bcc L8F46                          ; @8F25 90 1F
    bne L8F2F                          ; @8F27 D0 06
    ldx $4A                            ; @8F29 A6 4A
    cpx $46                            ; @8F2B E4 46
    bcc L8F46                          ; @8F2D 90 17
L8F2F:
    tax                                ; @8F2F AA
    lda $4A                            ; @8F30 A5 4A
    sbc $46                            ; @8F32 E5 46
    sta $4A                            ; @8F34 85 4A
    txa                                ; @8F36 8A
    sbc $47                            ; @8F37 E5 47
    bcs L8F46                          ; @8F39 B0 0B
L8F3B:
    tax                                ; @8F3B AA
    lda $4A                            ; @8F3C A5 4A
    sbc $46                            ; @8F3E E5 46
    sta $4A                            ; @8F40 85 4A
    txa                                ; @8F42 8A
    sbc $47                            ; @8F43 E5 47
    sec                                ; @8F45 38
L8F46:
    rol $4E                            ; @8F46 26 4E
    rol $4F                            ; @8F48 26 4F
    rol $4A                            ; @8F4A 26 4A
    rol a                              ; @8F4C 2A
    bcs L8F67                          ; @8F4D B0 18
    cmp $47                            ; @8F4F C5 47
    bcc L8F72                          ; @8F51 90 1F
    bne L8F5B                          ; @8F53 D0 06
    ldx $4A                            ; @8F55 A6 4A
    cpx $46                            ; @8F57 E4 46
    bcc L8F72                          ; @8F59 90 17
L8F5B:
    tax                                ; @8F5B AA
    lda $4A                            ; @8F5C A5 4A
    sbc $46                            ; @8F5E E5 46
    sta $4A                            ; @8F60 85 4A
    txa                                ; @8F62 8A
    sbc $47                            ; @8F63 E5 47
    bcs L8F72                          ; @8F65 B0 0B
L8F67:
    tax                                ; @8F67 AA
    lda $4A                            ; @8F68 A5 4A
    sbc $46                            ; @8F6A E5 46
    sta $4A                            ; @8F6C 85 4A
    txa                                ; @8F6E 8A
    sbc $47                            ; @8F6F E5 47
    sec                                ; @8F71 38
L8F72:
    rol $4E                            ; @8F72 26 4E
    rol $4F                            ; @8F74 26 4F
    rol $4A                            ; @8F76 26 4A
    rol a                              ; @8F78 2A
    bcs L8F93                          ; @8F79 B0 18
    cmp $47                            ; @8F7B C5 47
    bcc L8F9E                          ; @8F7D 90 1F
    bne L8F87                          ; @8F7F D0 06
    ldx $4A                            ; @8F81 A6 4A
    cpx $46                            ; @8F83 E4 46
    bcc L8F9E                          ; @8F85 90 17
L8F87:
    tax                                ; @8F87 AA
    lda $4A                            ; @8F88 A5 4A
    sbc $46                            ; @8F8A E5 46
    sta $4A                            ; @8F8C 85 4A
    txa                                ; @8F8E 8A
    sbc $47                            ; @8F8F E5 47
    bcs L8F9E                          ; @8F91 B0 0B
L8F93:
    tax                                ; @8F93 AA
    lda $4A                            ; @8F94 A5 4A
    sbc $46                            ; @8F96 E5 46
    sta $4A                            ; @8F98 85 4A
    txa                                ; @8F9A 8A
    sbc $47                            ; @8F9B E5 47
    sec                                ; @8F9D 38
L8F9E:
    rol $4E                            ; @8F9E 26 4E
    rol $4F                            ; @8FA0 26 4F
    rol $4A                            ; @8FA2 26 4A
    rol a                              ; @8FA4 2A
    bcs L8FBF                          ; @8FA5 B0 18
    cmp $47                            ; @8FA7 C5 47
    bcc L8FCA                          ; @8FA9 90 1F
    bne L8FB3                          ; @8FAB D0 06
    ldx $4A                            ; @8FAD A6 4A
    cpx $46                            ; @8FAF E4 46
    bcc L8FCA                          ; @8FB1 90 17
L8FB3:
    tax                                ; @8FB3 AA
    lda $4A                            ; @8FB4 A5 4A
    sbc $46                            ; @8FB6 E5 46
    sta $4A                            ; @8FB8 85 4A
    txa                                ; @8FBA 8A
    sbc $47                            ; @8FBB E5 47
    bcs L8FCA                          ; @8FBD B0 0B
L8FBF:
    tax                                ; @8FBF AA
    lda $4A                            ; @8FC0 A5 4A
    sbc $46                            ; @8FC2 E5 46
    sta $4A                            ; @8FC4 85 4A
    txa                                ; @8FC6 8A
    sbc $47                            ; @8FC7 E5 47
    sec                                ; @8FC9 38
L8FCA:
    rol $4E                            ; @8FCA 26 4E
    rol $4F                            ; @8FCC 26 4F
    rol $4A                            ; @8FCE 26 4A
    rol a                              ; @8FD0 2A
    bcs L8FEB                          ; @8FD1 B0 18
    cmp $47                            ; @8FD3 C5 47
    bcc L8FF6                          ; @8FD5 90 1F
    bne L8FDF                          ; @8FD7 D0 06
    ldx $4A                            ; @8FD9 A6 4A
    cpx $46                            ; @8FDB E4 46
    bcc L8FF6                          ; @8FDD 90 17
L8FDF:
    tax                                ; @8FDF AA
    lda $4A                            ; @8FE0 A5 4A
    sbc $46                            ; @8FE2 E5 46
    sta $4A                            ; @8FE4 85 4A
    txa                                ; @8FE6 8A
    sbc $47                            ; @8FE7 E5 47
    bcs L8FF6                          ; @8FE9 B0 0B
L8FEB:
    tax                                ; @8FEB AA
    lda $4A                            ; @8FEC A5 4A
    sbc $46                            ; @8FEE E5 46
    sta $4A                            ; @8FF0 85 4A
    txa                                ; @8FF2 8A
    sbc $47                            ; @8FF3 E5 47
    sec                                ; @8FF5 38
L8FF6:
    rol $4E                            ; @8FF6 26 4E
    rol $4F                            ; @8FF8 26 4F
    rol $4A                            ; @8FFA 26 4A
    rol a                              ; @8FFC 2A
    bcs L9017                          ; @8FFD B0 18
    cmp $47                            ; @8FFF C5 47
    bcc L9022                          ; @9001 90 1F
    bne L900B                          ; @9003 D0 06
    ldx $4A                            ; @9005 A6 4A
    cpx $46                            ; @9007 E4 46
    bcc L9022                          ; @9009 90 17
L900B:
    tax                                ; @900B AA
    lda $4A                            ; @900C A5 4A
    sbc $46                            ; @900E E5 46
    sta $4A                            ; @9010 85 4A
    txa                                ; @9012 8A
    sbc $47                            ; @9013 E5 47
    bcs L9022                          ; @9015 B0 0B
L9017:
    tax                                ; @9017 AA
    lda $4A                            ; @9018 A5 4A
    sbc $46                            ; @901A E5 46
    sta $4A                            ; @901C 85 4A
    txa                                ; @901E 8A
    sbc $47                            ; @901F E5 47
    sec                                ; @9021 38
L9022:
    rol $4E                            ; @9022 26 4E
    rol $4F                            ; @9024 26 4F
    rol $4A                            ; @9026 26 4A
    rol a                              ; @9028 2A
    bcs L9043                          ; @9029 B0 18
    cmp $47                            ; @902B C5 47
    bcc L904E                          ; @902D 90 1F
    bne L9037                          ; @902F D0 06
    ldx $4A                            ; @9031 A6 4A
    cpx $46                            ; @9033 E4 46
    bcc L904E                          ; @9035 90 17
L9037:
    tax                                ; @9037 AA
    lda $4A                            ; @9038 A5 4A
    sbc $46                            ; @903A E5 46
    sta $4A                            ; @903C 85 4A
    txa                                ; @903E 8A
    sbc $47                            ; @903F E5 47
    bcs L904E                          ; @9041 B0 0B
L9043:
    tax                                ; @9043 AA
    lda $4A                            ; @9044 A5 4A
    sbc $46                            ; @9046 E5 46
    sta $4A                            ; @9048 85 4A
    txa                                ; @904A 8A
    sbc $47                            ; @904B E5 47
    sec                                ; @904D 38
L904E:
    rol $4E                            ; @904E 26 4E
    rol $4F                            ; @9050 26 4F
    rol $4A                            ; @9052 26 4A
    rol a                              ; @9054 2A
    bcs L906F                          ; @9055 B0 18
    cmp $47                            ; @9057 C5 47
    bcc L907A                          ; @9059 90 1F
    bne L9063                          ; @905B D0 06
    ldx $4A                            ; @905D A6 4A
    cpx $46                            ; @905F E4 46
    bcc L907A                          ; @9061 90 17
L9063:
    tax                                ; @9063 AA
    lda $4A                            ; @9064 A5 4A
    sbc $46                            ; @9066 E5 46
    sta $4A                            ; @9068 85 4A
    txa                                ; @906A 8A
    sbc $47                            ; @906B E5 47
    bcs L907A                          ; @906D B0 0B
L906F:
    tax                                ; @906F AA
    lda $4A                            ; @9070 A5 4A
    sbc $46                            ; @9072 E5 46
    sta $4A                            ; @9074 85 4A
    txa                                ; @9076 8A
    sbc $47                            ; @9077 E5 47
    sec                                ; @9079 38
L907A:
    rol $4E                            ; @907A 26 4E
    rol $4F                            ; @907C 26 4F
    rol $4A                            ; @907E 26 4A
    rol a                              ; @9080 2A
    bcs L909B                          ; @9081 B0 18
    cmp $47                            ; @9083 C5 47
    bcc L90A6                          ; @9085 90 1F
    bne L908F                          ; @9087 D0 06
    ldx $4A                            ; @9089 A6 4A
    cpx $46                            ; @908B E4 46
    bcc L90A6                          ; @908D 90 17
L908F:
    tax                                ; @908F AA
    lda $4A                            ; @9090 A5 4A
    sbc $46                            ; @9092 E5 46
    sta $4A                            ; @9094 85 4A
    txa                                ; @9096 8A
    sbc $47                            ; @9097 E5 47
    bcs L90A6                          ; @9099 B0 0B
L909B:
    tax                                ; @909B AA
    lda $4A                            ; @909C A5 4A
    sbc $46                            ; @909E E5 46
    sta $4A                            ; @90A0 85 4A
    txa                                ; @90A2 8A
    sbc $47                            ; @90A3 E5 47
    sec                                ; @90A5 38
L90A6:
    rol $4E                            ; @90A6 26 4E
    rol $4F                            ; @90A8 26 4F
    rol $4A                            ; @90AA 26 4A
    rol a                              ; @90AC 2A
    bcs L90C7                          ; @90AD B0 18
    cmp $47                            ; @90AF C5 47
    bcc L90D2                          ; @90B1 90 1F
    bne L90BB                          ; @90B3 D0 06
    ldx $4A                            ; @90B5 A6 4A
    cpx $46                            ; @90B7 E4 46
    bcc L90D2                          ; @90B9 90 17
L90BB:
    tax                                ; @90BB AA
    lda $4A                            ; @90BC A5 4A
    sbc $46                            ; @90BE E5 46
    sta $4A                            ; @90C0 85 4A
    txa                                ; @90C2 8A
    sbc $47                            ; @90C3 E5 47
    bcs L90D2                          ; @90C5 B0 0B
L90C7:
    tax                                ; @90C7 AA
    lda $4A                            ; @90C8 A5 4A
    sbc $46                            ; @90CA E5 46
    sta $4A                            ; @90CC 85 4A
    txa                                ; @90CE 8A
    sbc $47                            ; @90CF E5 47
    sec                                ; @90D1 38
L90D2:
    rol $4E                            ; @90D2 26 4E
    rol $4F                            ; @90D4 26 4F
    sta $4B                            ; @90D6 85 4B
    clc                                ; @90D8 18
    rts                                ; @90D9 60
L90DA:
    lda #$00                           ; @90DA A9 00
    sta $4E                            ; @90DC 85 4E
    sta $4F                            ; @90DE 85 4F
    sec                                ; @90E0 38
    rts                                ; @90E1 60
; ---- executable island $9200 ----
* = $9200
L9200:
    lda $43                            ; @9200 A5 43
    bmi L9278                          ; @9202 30 74
    bne L9216                          ; @9204 D0 10
    lda $42                            ; @9206 A5 42
    bne L920D                          ; @9208 D0 03
    jmp L92E2                          ; @920A 4C E2 92
L920D:
    sta $46                            ; @920D 85 46
    lda #$00                           ; @920F A9 00
    sta $47                            ; @9211 85 47
    jmp L921C                          ; @9213 4C 1C 92
L9216:
    sta $47                            ; @9216 85 47
    lda $42                            ; @9218 A5 42
    sta $46                            ; @921A 85 46
L921C:
    lda $41                            ; @921C A5 41
    bmi L9234                          ; @921E 30 14
    lda $3E                            ; @9220 A5 3E
    sta $4C                            ; @9222 85 4C
    lda $3F                            ; @9224 A5 3F
    sta $4D                            ; @9226 85 4D
    lda $40                            ; @9228 A5 40
    sta $44                            ; @922A 85 44
    lda $41                            ; @922C A5 41
    sta $45                            ; @922E 85 45
    jsr L8E00                          ; @9230 20 00 8E
    rts                                ; @9233 60
L9234:
    lda #$00                           ; @9234 A9 00
    sec                                ; @9236 38
    sbc $3E                            ; @9237 E5 3E
    sta $4C                            ; @9239 85 4C
    lda #$00                           ; @923B A9 00
    sbc $3F                            ; @923D E5 3F
    sta $4D                            ; @923F 85 4D
    lda #$00                           ; @9241 A9 00
    sbc $40                            ; @9243 E5 40
    sta $44                            ; @9245 85 44
    lda #$00                           ; @9247 A9 00
    sbc $41                            ; @9249 E5 41
    sta $45                            ; @924B 85 45
    jsr L8E00                          ; @924D 20 00 8E
    lda #$00                           ; @9250 A9 00
    sec                                ; @9252 38
    sbc $4A                            ; @9253 E5 4A
    sta $4A                            ; @9255 85 4A
    lda #$00                           ; @9257 A9 00
    sbc $4B                            ; @9259 E5 4B
    sta $4B                            ; @925B 85 4B
    lda #$00                           ; @925D A9 00
    sec                                ; @925F 38
    sbc $4E                            ; @9260 E5 4E
    sta $4E                            ; @9262 85 4E
    lda #$00                           ; @9264 A9 00
    sbc $4F                            ; @9266 E5 4F
    sta $4F                            ; @9268 85 4F
    lda #$00                           ; @926A A9 00
    sbc $48                            ; @926C E5 48
    sta $48                            ; @926E 85 48
    lda #$00                           ; @9270 A9 00
    sbc $49                            ; @9272 E5 49
    sta $49                            ; @9274 85 49
    clc                                ; @9276 18
    rts                                ; @9277 60
L9278:
    lda #$00                           ; @9278 A9 00
    sec                                ; @927A 38
    sbc $42                            ; @927B E5 42
    sta $46                            ; @927D 85 46
    lda #$00                           ; @927F A9 00
    sbc $43                            ; @9281 E5 43
    sta $47                            ; @9283 85 47
    lda $41                            ; @9285 A5 41
    bmi L92B7                          ; @9287 30 2E
    lda $3E                            ; @9289 A5 3E
    sta $4C                            ; @928B 85 4C
    lda $3F                            ; @928D A5 3F
    sta $4D                            ; @928F 85 4D
    lda $40                            ; @9291 A5 40
    sta $44                            ; @9293 85 44
    lda $41                            ; @9295 A5 41
    sta $45                            ; @9297 85 45
    jsr L8E00                          ; @9299 20 00 8E
    lda #$00                           ; @929C A9 00
    sec                                ; @929E 38
    sbc $4E                            ; @929F E5 4E
    sta $4E                            ; @92A1 85 4E
    lda #$00                           ; @92A3 A9 00
    sbc $4F                            ; @92A5 E5 4F
    sta $4F                            ; @92A7 85 4F
    lda #$00                           ; @92A9 A9 00
    sbc $48                            ; @92AB E5 48
    sta $48                            ; @92AD 85 48
    lda #$00                           ; @92AF A9 00
    sbc $49                            ; @92B1 E5 49
    sta $49                            ; @92B3 85 49
    clc                                ; @92B5 18
    rts                                ; @92B6 60
L92B7:
    lda #$00                           ; @92B7 A9 00
    sec                                ; @92B9 38
    sbc $3E                            ; @92BA E5 3E
    sta $4C                            ; @92BC 85 4C
    lda #$00                           ; @92BE A9 00
    sbc $3F                            ; @92C0 E5 3F
    sta $4D                            ; @92C2 85 4D
    lda #$00                           ; @92C4 A9 00
    sbc $40                            ; @92C6 E5 40
    sta $44                            ; @92C8 85 44
    lda #$00                           ; @92CA A9 00
    sbc $41                            ; @92CC E5 41
    sta $45                            ; @92CE 85 45
    jsr L8E00                          ; @92D0 20 00 8E
    lda #$00                           ; @92D3 A9 00
    sec                                ; @92D5 38
    sbc $4A                            ; @92D6 E5 4A
    sta $4A                            ; @92D8 85 4A
    lda #$00                           ; @92DA A9 00
    sbc $4B                            ; @92DC E5 4B
    sta $4B                            ; @92DE 85 4B
    clc                                ; @92E0 18
    rts                                ; @92E1 60
L92E2:
    sta $46                            ; @92E2 85 46
    sta $47                            ; @92E4 85 47
    jmp L8E00                          ; @92E6 4C 00 8E
